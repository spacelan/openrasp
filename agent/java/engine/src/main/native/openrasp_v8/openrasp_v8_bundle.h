...
public:
  ...
  Snapshot(const std::string &config, const std::vector<PluginFile> &plugin_list, uint64_t timestamp = 0, void *data = nullptr);
  ~Snapshot();
...
void plugin_info(Isolate *isolate, const char *message, size_t length);
...
inline void log_callback(const v8::FunctionCallbackInfo<v8::Value> &info)
{
  ...
  for (int i = 0; i < info.Length(); i++)
  {
    ...
    plugin_info(isolate, *message, message.length());
  }
}
inline void plugin_info(Isolate *isolate, const std::string &message)
{
  plugin_info(isolate, message.c_str(), message.length());
}
